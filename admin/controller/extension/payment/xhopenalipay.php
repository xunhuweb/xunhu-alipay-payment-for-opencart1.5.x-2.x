...
class Controllerextensionpaymentxhopenalipay extends Controller
{
    ...
    public function index()
    {
        ...
        if($this->is_version_2300()){
           $url_payment= $this->url->link('payment/xhopenalipay', 'token=' . $this->session->data['token'], 'SSL');
        }else{
        ...
        if($this->is_version_2300()){
            ...
            $data['action'] = $this->url->link('payment/xhopenalipay', 'token=' . $this->session->data['token'], 'SSL');
        }
        ...
        if(version_compare(VERSION,'2.0.0.0', '>=')){
            ...
            $this->data = $data;
            $this->template = 'payment/xhopenalipay.tpl';
            $this->children = array(
                'common/header',
                'common/footer'
            );
    ...
    {
        if (! $this->user->hasPermission('modify', $this->is_version_2300()?'extension/payment/xhopenalipay':'payment/xhopenalipay')) {
